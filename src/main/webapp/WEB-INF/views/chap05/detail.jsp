...
<body>
    ...
    <div id="wrap" class="form-container">
        ...
        <div class="buttons">
            <div class="update-btn" type="button" onclick="window.location.hred='/board/update'">수정</div>
        </div>
           ...
           // 페이지 클릭 이벤트 핸들러
            function makePageButtonClickEvent() {
                ...
                $pageUl.onclick = e => {
                    ...
                    e.preventDefault(); // 태그의 기본 동작 중단 (a태그의 링크이동 기능 막음)
                    // 누른 페이지 번호 가져오기
                    ...
                    fetch(URL, requestInfo)
                        .then(res => {
                            if (res.status === 200) {
                                ...
                                // getReplyList(1);
                                // 마지막페이지 번호
                                const lastePageNo = document.querySelector('.pagination').dataset.fp;
                                getReplyList(lastePageNo);
                            } else {
                                alert('댓글 등록에 실패함!');
                            }
                        })
            ...
            function replyRemoveClickEvent() {
                ...
                $replyData.onclick = e => {
                    ...
                    if(e.target.matches('#replyDelBtn')) {
                        ...
                        fetch(URL + '/' + rno, {
                            method: 'DELETE'
                            // headers: 바디에 실어보낼 게 없어서 헤더 그냥 기본헤더 보내면 됨
                        }).then(res => {
                            if(res.status===200) {
                                alert('댓글 정상 삭제');
                                return res.json();
                        ...
                        // 모달에 수정버튼 누른 댓글번호도 같이 몰래 숨겨 놓기 (->그래야 나중에 수정화면에서 삭제버튼누르면 어떤 번호인지 앎)
                        // 다음 수정완료 처리를 위해 미리 수정창을 띄울 때
